...
import { submitTransaction } from "@/actions/transaction-actions-fixed"
...
function TransactionFormContent({
  ...
  // Debug mode and info removed for production
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentStep === 7) {
      ...
      try {
        ...
        // Add to debug info
        if (debugMode) {
          setDebugInfo((prev: Record<string, unknown> | null) => ({
            ...(prev || {}),
            submissionData
          }))
        }
        ...
        try {
          ...
          try {
            ...
            if (debugMode) {
              setDebugInfo((prev: Record<string, unknown> | null) => ({
                ...(prev || {}),
                minimalError: minimalError instanceof Error ?
                  { message: minimalError.message, stack: minimalError.stack } :
                  String(minimalError)
              }))
            }
          ...
          // Now try the actual submission with a simplified approach
            ...
            if (debugMode) {
              setDebugInfo((prev: Record<string, unknown> | null) => ({
                ...
                submissionError: submissionError instanceof Error ?
                  { message: submissionError.message, stack: submissionError.stack } :
          ...
          if (result && result.success) {
            ...
            if (debugMode) {
              ...
              // Redirect to success page if not in debug mode
              // Pass the transaction ID if available
            ...
            console.error("Transaction submission error:", {
              result,
              submissionData: { ...submissionData, transaction_value: submissionData.transaction_value }
            });
    ...
    <>
      ...
      {debugMode && debugInfo && (
        <div className="mt-8 p-4 border rounded-lg bg-slate-50">
          ...
          <pre className="text-xs overflow-auto max-h-96 p-2 bg-slate-100 rounded">
